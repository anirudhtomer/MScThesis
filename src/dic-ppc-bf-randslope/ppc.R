ppcCheck=foreach(k=1:nrow(mcmcfit[[1]]),.combine='rbind', .packages='MASS') %dopar%{
  ...
  for(j in 1:nsubjects){
    ...
    #sampleRandomPart = randComp[j, 1] + (randComp[j, 2]*time + rnorm(n = nrep, sd = errSd, mean = 0))
...
temp = c()
for(i in 1:nrow(mcmcfit_5wellsep6comp[[1]])){
  temp[i]= getEta(mcmcfit_5wellsep6comp, mcmcIterNum=i)[4]
}
