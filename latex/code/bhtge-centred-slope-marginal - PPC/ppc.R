...
mahalaDist=foreach(i=1:mcmcLen, .combine='rbind') %do%{
  ...
  randomPartMu=t(matrix(c(rep(1,nrep),c(1:nrep)), nrow = nrep, ncol=2)%*%t(getRandMu(mcmcfit, mcmcIterNum = 1)))
  ...
  for(j in 1:nsubjects){
    ...
    for(m in 1:ncomponents){
      if(alloc!=m){
        obsMahal[j] = obsMahal[j] + mahalanobis(randomPart, randomPartMu[m,], 
                                                sigmaEst)
      }
    }
